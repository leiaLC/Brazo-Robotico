...
Topics publicados:
    ...
    /perception/yolo/debug_image (sensor_msgs/Image)  ← visual para RViz

Parámetros (yolo_params.yaml):
    model_path   — ruta al modelo .pt  (debe ser un modelo *-seg, ej. yolov8n-seg.pt)
    confidence   — umbral de confianza  (default 0.5)
    device       — 'cpu' o 'cuda:0'
    fx, fy, cx, cy, distortion — intrínsecos calibrados
"""
...
class YoloNode(Node):
    ...
    def __init__(self):
        ...
        # ── Parámetros ───────────────────────────────────────────────────
        self.declare_parameter('model_path', 'yolov8n-seg.pt')   # modelo seg
        self.declare_parameter('confidence', 0.5)
        self.declare_parameter('device',     'cuda:0')  # o 'cpu'

        # Intrínsecos para undistort
        ...
        self.new_camera_matrix = None   # se inicializa en el primer frame
        self.roi               = None

        # ── YOLO ─────────────────────────────────────────────────────────
        # Asegúrate de usar un modelo -seg (yolov8n-seg, yolov8s-seg, etc.)
        self.model = YOLO(model_path)
        self.model.to(device)

        if 'seg' not in model_path:
            self.get_logger().warn(
                f'El modelo "{model_path}" no parece ser un modelo de segmentación. '
        ...
        self.get_logger().info(
            f'YoloNode listo — modelo: {model_path} | '
            f'conf: {self.conf} | device: {device}'
        )
    ...
    def callback(self, rgb_msg: Image) -> None:
        ...
        boxes = result.boxes
        masks = result.masks   # None si no hay detecciones o modelo no es -seg
        ...
        for i, box in enumerate(boxes):
            ...
            # Campos 3D vacíos — los rellena depth_estimator_node
            det.x           = 0.0
            det.y           = 0.0
            det.z           = 0.0
            ...
            if masks is not None and i < len(masks):
                # masks.data[i] → tensor (mask_h, mask_w) con valores 0-1
            ...
            self.get_logger().debug(
                ...
                f'mask={det.mask_width}x{det.mask_height}'
            )
